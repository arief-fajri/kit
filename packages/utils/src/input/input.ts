 ...
 */
export function formatNumberLocale(value: number | string, locale: string = "en-US", useFormatting: boolean = true): string {
...
export function parseAndFormatNumber(
  ...
  if (intDigits.length > 0) {
    ...
    if (!isNaN(numInt) && numInt >= 0) {
      ...
      if (intDigits.length >= 4) {
        formattedInt = numInt.toLocaleString(locale, { useGrouping: true, maximumFractionDigits: 0 });
      } else {
  ...
  const numericString = (isNegative ? '-' : '') + intDigits + (decimalPart ? '.' + decimalPart : '');
