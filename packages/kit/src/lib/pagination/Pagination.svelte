<script lang="ts">
	...
	function generateCustomStyles(): string {
		const styles: string[] = [];
		if (computedStyling.backgroundColor)
			styles.push(`--pagination-bg: ${computedStyling.backgroundColor}`);
		if (computedStyling.textColor) styles.push(`--pagination-text: ${computedStyling.textColor}`);
		if (computedStyling.borderColor)
			styles.push(`--pagination-border: ${computedStyling.borderColor}`);
		if (computedStyling.activeBackgroundColor)
			styles.push(`--pagination-active-bg: ${computedStyling.activeBackgroundColor}`);
		if (computedStyling.activeTextColor)
			styles.push(`--pagination-active-text: ${computedStyling.activeTextColor}`);
		if (computedStyling.borderRadius)
			styles.push(`--pagination-border-radius: ${computedStyling.borderRadius}`);
	...
	$: pageNumbers = calculatePageNumbers(
		...
		totalPages,
		safeVisiblePages,
		computedBehavior.showEllipsis ?? false
	);
	$: customStyles = generateCustomStyles();
	$: pageInfo = computedBehavior.showPageInfo
		? formatPageInfo(
				safeCurrentOffset,
				safeLimit,
				safeTotalRows,
				computedBehavior.pageInfoFormatter
			)
		: '';
	...
	$: paginationClassesWithModifiers =
		`${paginationClasses}${computedBehavior.disabled ? ' pagination--disabled' : ''}`.trim();
</script>

<nav
	class={paginationClassesWithModifiers}
...
<style lang="postcss">
  ...
  */
	...
	.pagination {
		...
		--pagination-bg-active: var(
			--pagination-bg-active-default,
		...
		--pagination-text-disabled: var(
			--pagination-text-disabled-default,
			var(--color-text-muted, #9ca3af)
		);

		/* Colors - Active State */
		--pagination-active-bg: var(--pagination-active-bg-default, var(--color-primary-500, #3b82f6));
		--pagination-active-text: var(
			--pagination-active-text-default,
			var(--color-primary-text, #ffffff)
		);
		--pagination-active-bg-hover: var(
			--pagination-active-bg-hover-default,
		...
		--pagination-border-active: var(
			--pagination-border-active-default,
