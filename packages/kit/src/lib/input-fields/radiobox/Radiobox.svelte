...
>
	<label
		class="radiobox-container {computedStyling.labelClass}"
		class:radiobox-container--rtl={computedBehavior.rtl}
	>
		<input
			type="radio"
...
<style>
	...
	/* Hover state */
	.radiobox-container:hover:not(:has(.radiobox-input:disabled))
		.radiobox-indicator:not(.radiobox-indicator--checked) {
		border-color: var(--current-color, var(--radiobox-primary, var(--color-primary, #3b82f6)));
	}
