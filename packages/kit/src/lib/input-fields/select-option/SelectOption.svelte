<script lang="ts">
	...
	function handleSelect(option: SelectOptionItem) {
		...
		const updateValue = computedBehavior.isMultiOption
			? listValueSelected
			: listValueSelected[0] || null;
					...
					{:else if listOptions?.length}
						{#each listOptions as item, index}
							<slot
								name="option"
								{item}
								isSelected={listValueSelected.some((sel) => sel.value === item.value)}
							>
								<div
									...
									class:select-option__item--selected={listValueSelected.some(
...
<style lang="postcss">
	...
	.select-option__trigger {
		...
		border: var(--select-option-border-width, 1px) solid
			var(--select-option-border, var(--color-border, #e5e7eb));
