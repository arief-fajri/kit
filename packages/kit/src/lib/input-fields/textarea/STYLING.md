...
```svelte
<TextArea label="Dynamic Theme" styling={{ wrapperStyle: textareaStyles }} />
```
...
.textarea-glow:focus-within {
	box-shadow:
		0 0 0 1px var(--textarea-border-focus),
...
```svelte
<!-- Usage in components -->
<TextArea label="Primary Action" styling={{ wrapperClass: 'textarea-primary' }} />
