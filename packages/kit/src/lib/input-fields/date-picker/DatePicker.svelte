<script lang="ts">
	...
	const handleDateChange = (
		event: CustomEvent<{ date: Date; type: 'select' | 'range-start' | 'range-end' }>
	) => {
	...
	$: rootClasses = [
		'date-picker',
		`date-picker--${computedStyling.variant}`,
		`date-picker--${computedStyling.size}`,
