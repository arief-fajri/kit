<script lang="ts">
	...
	$: normalizedColumns = columns.map((col) =>
		normalizeTableColumn(col as NormalizeTableColumnInput)
	) as TableColumn[];
	...
	$: paginatedData = computedBehavior.serverSide
		? data
		: sortedData.slice(currentOffset ?? 0, (currentOffset ?? 0) + (limit ?? 10));

	// Reset offset when sort changes
	let previousSort = sort;
	$: {
		if (
			computedBehavior.autoResetOffsetOnSort &&
			previousSort !== sort &&
			!computedBehavior.loading
		) {
			currentOffset = 0;
			previousSort = sort;
		}
	...
	const getRowClass = (row: TableRow, index: number): string => {
		...
		if (dragOverRowIndex === index && draggedRowIndex !== index) {
			const isValidDrop =
				computedBehavior.canDrop && draggedRowData
					? computedBehavior.canDrop(draggedRowData, row)
					: true;
	...
	$: hoverStyle = computedStyling.hoverColor
		? `background-color: ${computedStyling.hoverColor};`
		: undefined;
	...
	const handleDragOver = (event: DragEvent, row: TableRow, index: number) => {
		...
		const isValidDrop = computedBehavior.canDrop
			? computedBehavior.canDrop(draggedRowData!, row)
			: true;
	...
	const handleDrop = (event: DragEvent, targetRow: TableRow, targetIndex: number) => {
		...
		const isValidDrop = computedBehavior.canDrop
			? computedBehavior.canDrop(draggedRowData!, targetRow)
			: true;
...
<div
	class="table-listing bg-[--color-surface] rounded-lg overflow-hidden border border-[--color-border] {computedStyling.tableContainerClass ||
		computedStyling.className}"
...
>
	<div class="table-listing__scroll-wrapper">
		...
		>
			...
			<tbody
				class="table-listing__body divide-y divide-[--color-border] {computedStyling.tbodyClass}"
			>
	...
	{#if computedBehavior.showPagination && computedTotalRows > 0 && !computedBehavior.loading}
		<div
			class="table-listing__pagination px-4 py-4 border-t flex items-center justify-center {computedStyling.paginationClass}"
		>
			<Pagination
				bind:currentOffset
				{limit}
