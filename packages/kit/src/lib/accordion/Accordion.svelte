	...
	<div class="accordion__header-wrapper {computedStyling.headerWrapperClass}">
		...
		>
			{#if computedBehavior.showIcon && computedBehavior.iconPosition === 'left'}
				<div
					class="accordion__icon {computedStyling.iconClass}"
					class:accordion__icon--rotated={isExpanded}
				>
					<svg
...
<style lang="postcss">
	...
	/* Header button */
	.accordion__header {
		padding: var(--accordion-padding-x, var(--accordion-padding-x-md))
			var(--accordion-padding-x, var(--accordion-padding-x-md));
		border: none;
		...
		gap: 0.5rem;
		color: var(--accordion-text, var(--color-text, #111827));
	...
	.accordion__header:hover:not(:disabled) {
		color: var(--accordion-text-hover, var(--color-primary, #3b82f6));
	}
	...
	.accordion__header--underline {
		border-bottom: var(--accordion-border-width) solid
			var(--accordion-border-color, var(--color-border, #d1d5db));
	}
	...
	.accordion__subtitle {
		...
		font-style: italic;
		line-height: var(--accordion-line-height);
		color: var(--accordion-text-subtitle, var(--color-text-muted, #6b7280));
	}
	...
	.accordion__icon {
		...
		color: var(--accordion-icon-color, var(--color-text, #111827));
		transition:
			transform var(--accordion-transition-duration) var(--accordion-transition-easing),
	...
	.accordion__header:hover:not(:disabled) .accordion__icon {
		color: var(--accordion-icon-color-hover, var(--color-primary, #3b82f6));
	}
